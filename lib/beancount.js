...
module.exports = {
	...
	activate(state) {
		...
		this.subscriptions.add(atom.commands.add('atom-workspace', {
			'beancount:check': () => this.beancheck()
		}));
		this.subscriptions.add(atom.commands.add('atom-workspace', {
			'beancount:fava': () => this.beanfava()
		}));
		this.subscriptions.add(atom.commands.add('atom-workspace', {
			'beancount:query': () => this.beanquery()
		}));
	},
	...
	beanformat() {
		...
		if(editor) {
			...
			tmp.file(function (err, path, fd, cleanup) {
				...
				const stdout = (output) => {
					if (output !== editor.getBuffer().getText()) {
						editor.getBuffer().setText(output);
	...
	beancheck() {
		...
		if(editor) {
			...
			let ledger = atom.config.get('beancount-pulsar.ledger');
