...
module.exports = {
	...
	beanquery(ledger) {
		...
		if (format == 'Prompt') {
			let prompt = atom.notifications.addInfo("Choose what format to export to:", {
				buttons: [
					{
						text: "csv",
						onDidClick: () => { this.beanqueryRun('csv', ledger, query); prompt.dismiss(); }
					},
					{
						text: "text",
						onDidClick: () => { this.beanqueryRun('text', ledger, query); prompt.dismiss(); }
					}
				],
				dismissable: true
			});
		} else {
			this.beanqueryRun(format, ledger, query);
		}
	},

	beanqueryRun(format, ledger, query) {
		const { BufferedProcess } = require('atom');
